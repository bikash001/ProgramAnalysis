...
void Liveness::computeLiveness(Function &F) {
	...
	std::set<std::string> live_at_print(print_block_data.out.begin(), print_block_data.out.end());
	std::set<std::string> def, use;
	std::map<Value*, std::set<std::string> > track_data;
				...
				case Instruction::Store:
					{
						...
						if (val->hasName()) {
							def.insert(val->getName());
							std::map<Value*,std::set<std::string> >::iterator it = track_data.find(dyn_cast<Value>(I->getOperand(0)));
				...
				case Instruction::Load:
					{
						...
						if (after_call) {
							...
							if (val->hasName()) {
								...
								temp.insert(str);
								track_data.insert(std::pair<Value*,std::set<std::string> > (dyn_cast<Value>(I), temp));
							}
						}
						break;
					}
				case Instruction::Call:
					{
						if (!after_call) {
							...
							if (f != NULL && fname.compare("printf") == 0) {
								after_call = true;
								std::map<Value*, std::set<std::string> >::iterator itr;
	...
	for (std::set<std::string>::iterator b=live_at_print.begin(), e=live_at_print.end(); b != e; ++b) {
		outs() << *b << " ";
	}
	outs()<< "\n";
}
